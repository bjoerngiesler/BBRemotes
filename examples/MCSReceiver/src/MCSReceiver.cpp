...
void timeoutCB(Protocol* proto, float timeout) {
  bb::rmt::printf("Timeout! No data received in %.2f seconds\n", timeout);
...
void setup() {
  ...
  // Tell the receiver about the two inputs our droid knows. These are names predefined by the library
  // but you can use any string.
  speedInput = rx->addInput(INPUT_NAME_SPEED, speed);
  turnInput = rx->addInput(INPUT_NAME_TURN_RATE, turn);
  ...
  // Tell the receiver to call timeoutCB() whenever no data is received for a certain time
   ...
   protocol.setCommTimeoutWatchdog(.5, timeoutCB);
}
